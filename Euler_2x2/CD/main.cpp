...
int main()
{
	...
	mesh M (xa, xb, dx);
	int N = M.get_N();
	double uL(0), uR(2.246510973213954), tauL(0.2), tauR(1), gamma(1.4), x_c(0.5);//uL(-1.563415104628313), uR(-3), tauL(0.2), tauR(0.5), gamma(1.4), x_c(0.5);
	double cfl(0.5);
	...
	ofstream file_u ("../../../results/Euler_2x2_CD/1-raref/u.dat");
	ofstream file_tau ("../../../results/Euler_2x2_CD/1-raref/tau.dat");
	ofstream file_s_u ("../../../results/Euler_2x2_CD/1-raref/s_u.dat");
	ofstream file_s_tau ("../../../results/Euler_2x2_CD/1-raref/s_tau.dat");
	ofstream file_t ("../../../results/Euler_2x2_CD/1-raref/t.dat");
	...
	while (t < T)
	{
		...
		for (int i = 1; i < N; ++i)
		{
			...
			{
				if (t > 0.04) {
				if(U[0][i] < U[0][i-1]) // 2-rarefaction
				{
					sigma[i] = lambda[i];
					s_sigma[i] = lambda[i];
				}
