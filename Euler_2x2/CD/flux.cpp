...
void flux::residual(state st, vector<vector<double> >& R, vector<vector<double> >& S)
{
	...
	for (int i = 0; i <= N; ++i)
	{
		...
		vector<double> FL(4), FR(4), Ustar(2);
		...
		for (int k = 0; k < 2; ++k)
		{
			...
			/* if(i < 253 && i > 247)
			{
				...
				cout << "Ustar[" << k+2 << "][" << i << "] = " << 0.5*(UL[k+2] + UR[k+2]) - 0.5*(FR[k+2]-FL[k+2])/lambdaR[i] - s_lambdaR[i]/lambdaR[i]*Ustar[k] + s_lambdaR[i]*0.5*(UL[k] + UR[k])/lambdaR[i] << endl;
			}*/
		}
