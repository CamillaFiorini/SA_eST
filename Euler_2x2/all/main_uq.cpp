...
int main()
{
	...
	mesh M (xa, xb, dx);
	string path("res_uq/riemann_pb/");
	int N (M.get_N());
	double uL(0), uR(0), tauL(0.7), tauR(0.2), gamma(1.4), x_c(0.5);
	double cfl(0.5);
	int NP = 4;
	vector<double> u0(N, uR);
	vector<double> tau0(N, tauR);
	vector<vector<double> > s_tau0(NP,u0), s_u0(NP,u0);
	cout.precision(15);
...
/*	double pi (4*atan(1)), L(0.25), m(0.05);
	...
	for (int k = 0; k < NP; ++k)
	{
		st[k].set_U(tau0,u0,s_tau0[k],s_u0[k]);
