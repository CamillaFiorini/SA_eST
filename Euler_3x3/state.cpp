...
void state::get_W(vector<vector<double> >& a) const
{
	...
	for (int k = 0; k < U[0].size(); ++k)
	{
		...
		a[2][k] = (gamma-1)*(U[2][k] - 0.5*U[1][k]*U[1][k]/U[0][k]); // p = (gamma-1)(E - 0.5(rho*u)^2/rho)
		a[4][k] = (U[4][k] - U[3][k]*a[1][k])/U[0][k]; // s_u
		a[5][k] = (gamma-1)*(U[5][k]-0.5*U[3][k]*a[1][k]*a[1][k] - U[0][k]*a[1][k]*a[4][k]); //s_p
	}
	return;
};
...
void state::flux(const vector<double>& u, vector<double>& F) const
{
	...
	F[4] = u[3]*u[1]*u[1] + 2*u[1]*s_u + (gamma-1)*(u[5]-0.5*u[3]*u[1]/u[0]*u[1]/u[0] - u[0]*u[1]/u[0]*s_u); //s_rho*u*u + 2*rho*u*s_u + s_p
	F[5] = s_u*(u[2] + (gamma-1)*(u[2] - 0.5*u[1]*u[1]/u[0])) + u[1]/u[0]*(u[5] + (gamma-1)*(u[5]-0.5*u[3]*u[1]/u[0]*u[1]/u[0] - u[0]*u[1]/u[0]*s_u)); //s_uL*(EL+pL) + uL*(s_EL+s_pL);
}