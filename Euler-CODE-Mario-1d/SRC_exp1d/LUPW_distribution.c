       ...
       for ( v = 0 ; v < 2 ; v ++ )
           {
            ...
            for ( k = 0 ; k < size ; k ++ )
                {
                  //phi_node[k] = DDV[v][k]  + theta*phi_w[k]/2. ; // for BcUPW
                  phi_node[k] = DDV[v][k] ; // for LUPW

		 // Projecting back + dissipation......
                  for ( l = 0 ; l < size ; l ++ )
                  //      phi_node[k] += ( 1. - theta )*Right[k][l]*work_vector1[l] + theta*K1[v][k][l]*phi_w[l] ; // for BcUPW
                        phi_node[k] += Right[k][l]*work_vector1[l] + theta*K1[v][k][l]*phi_w[l] ; // For LUPW
                }
